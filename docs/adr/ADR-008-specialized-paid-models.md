...
- **Triage** (`triage.py`, cheap structured classification): `openai/gpt-5-nano`, via new `get_triage_llm()` (`app/services/llm.py`). No model-level fallback — triage already degrades safely (JSON-parse retry, then defaults to `"rag"`) without needing a second model.
