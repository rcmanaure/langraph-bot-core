"""Convert the human-authored catalog Markdown into the JSONL shape
app/services/indexer.py's _extract_jsonl_chunks() expects.

Why this exists: sp-diagnostico-histologico.md is the lab admin's editable
source of truth (git-diffable, easy to update prices). The RAG pipeline
should never chunk that Markdown directly -- generic text/table splitters
detach price-table headers from rows and corrupt price/code associations
(this is exactly how medical_data/sp-diagnostico-histologico.jsonl drifted:
SOM002-SOM010 describe different procedures in the .md vs the stale .jsonl,
and PIEL Y ANEXOS CUTÁNEOS / ESTUDIOS CITOLÓGICOS ESPECÍFICOS are missing
from the .jsonl entirely). This script is the one place that parses the
.md, so every future edit to the .md and re-run of this script keeps the
ingested catalog aligned with the source of truth -- no more manual
